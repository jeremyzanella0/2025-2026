#Mark 4- Adds play by play commentary to the shot
...
# ---------- (kept) CLI helper imports ----------
import subprocess, sys, os  # for running pscoding18 as a CLI (unused now but left intact)
# ----------------------------------------------
...
def make_click_dots():
    ...
    # Dense grid covering exactly the court area
    x = 0.0
    while x <= COURT_W:
        y = 0.0
        while y <= HALF_H:
    ...
    print(f"Generated {len(xs)} dots covering {min(xs)}-{max(xs)} x {min(ys)}-{max(ys)}")
...
def base_fig():
    ...
    fig.update_layout(
        paper_bgcolor="white",
        plot_bgcolor="white",
        width=720, height=1000,  # larger chart
        xaxis=dict(
            range=[0, COURT_W], 
            showgrid=False, 
            zeroline=False,
...
app.title = "Shot Chart — click anywhere on the court to enter a possession"
...
# ====== LAYOUT (shot chart left, output right) ======
app.layout = html.Div([
    html.H2("Shot Chart — click anywhere on the court to enter a possession",
    ...
    html.Div([
        ...
        # RIGHT: Play-by-Play only
        html.Div([
            html.Div(id="debug_click",
                     style={"fontSize": "14px", "color": "#666", "margin": "0 0 8px 0"}),
    ...
    # Modal is outside the row so the overlay spans the whole page
    html.Div(
        id="input_modal",
...
def parse_and_render(btn_clicks, n_submit, possession_text, xy):
    ...
    # Location & Shot Distance (from hoop center) after play-by-play
    trailing = []
