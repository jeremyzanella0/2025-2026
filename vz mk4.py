# vz_mk4 — adds defensive / rebounding / special token parsing, adds zone defense parsing, adds offensive and defensive players lists parsing
...
# ========================= NEW: Special-stats display helpers =========================
_SPECIAL_LABELS = {
...
def collect_special_people(short_text: str):
    out = []
    for row in special_stats_for_display(short_text or ""):
        out.extend(row.get("players", []))
    seen = set(); uniq = []
    for n in out:
        k = (n or "").lower()
        if k and k not in seen:
...
# ========================= NEW: Block row from PBP (for details UI) =========================
def blockers_from_pbp_for_display(pbp_text: str) -> list[str]:
    """
    Convenience wrapper for Section-1's parse_blockers_from_pbp(...).
    Returns roster-normalized, de-duplicated list of blocker names.
    """
    try:
...
def special_stats_with_pbp_blocks(short_text: str, pbp_text: str):
    """
    Same shape as special_stats_for_display(...), but merges a 'Block' row
    derived from play-by-play text (e.g., 'Lusk blocks the shot').
    Shorthand-detected blocks are kept; PBP names are added and de-duplicated.
    """
    rows = special_stats_for_display(short_text or "")
    # Build label -> list map
    by_label = {r["label"]: list(r.get("players", []) or []) for r in rows}
    # Merge in blockers from PBP
    ...
    # Re-emit rows in a stable order
    ...
    for lbl in order:
        ...
        if players:
            # Recover the original key from _SPECIAL_LABELS
            key = None
...
# ========================= NEW (ADDED): Zone defense + participants wrappers =========================

def defense_label_for_shot(pbp_text: str, shot_index: int) -> str:
    """
    Wrapper for Section-1's zone_for_shot(...).
    Returns a string like '2-3 Zone' or 'Man to Man' for the given shot index.
    """
    try:
...
def participants_for_possession(pbp_text: str, shorthand_text: str):
    """
    Wrapper for Section-1's collect_players_from_possession(...).
    Returns (offensive_players, defensive_players) as lists of full names.
    """
    try:
...
# ================== NEW: sanitize trailing modifiers for participant lists ==================
...
def show_shot_details(clickData, close_clicks):
    ...
    try:
        ...
        # Prefer shot-line defender string
        ...
        # ===== NEW: Zone defense label + participants (offense/defense) =====
        try:
            def_label = defense_label_for_shot(pbp_src_for_roles, idx or 1) or "Man to Man"
        except Exception:
            def_label = "Man to Man"

        # OVERRIDE: If shorthand shows zone via [ ... ], use that (and capture type like 2-3 / 3-2 / 1-3-1)
        _short = (r.get("possession") or "")
        if "[" in _short and "]" in _short:
            m_zone = re.search(r"\b(\d(?:-\d){1,3})\s*\[", _short)
            def_label = f"{m_zone.group(1)} Zone" if m_zone else "Zone"
        ...
        # Normalize against possession roster for display (strip trailing modifiers first)
        op_list = []
        seen_op = set()
        for nm in (_op or []):
        ...
        header_top = html.Div([
            html.Div([
                ...
                html.Span(f" • Defense: {def_label}", style={"marginLeft":"6px","color":"#444"}),  # NEW
            ...
            # NEW: participants listed after Drill
            html.Div([
        ...
        header = html.Div(style={"display":"flex","justifyContent":"space-between","alignItems":"center"},
                          children=[header_top, header_btn])
        ...
        # ===== Special stats: parse shorthand -> names-only, then MERGE PBP inference =====
        specials_rows = special_stats_with_pbp_blocks(r.get("possession",""), pbp_src_for_roles) or []

        # FINAL display sanitizer: strip lingering verbs/phrases and expand to full roster names
        ...
        # Build label -> players map from shorthand + PBP (already merged)
        _spec_by_label = {}
        ...
        # PBP merge/backfill for Deflection & Steal (subject or passive "by" forms)
        ...
        if txt_for_infer:
            ...
            # --- NEW: PBP merge/backfill for FOULS (subject + passive "by" forms)
        ...
        # ---- Insert BLOCK right under Defender(s) (desired placement)
        ...
        for lbl in _special_order:
            if lbl == "Block":   # already inserted above in the identity table
                continue
            ppl = _spec_by_label.get(lbl, [])
            if ppl:
                ident_rows.append((lbl, ", ".join(ppl)))
        # ===== END special stats block =====
