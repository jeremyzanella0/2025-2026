...
def _ensure_player(stats, name):
    ...
    if name not in stats:
        stats[name] = {
            ...
            "STL":0,"DEF":0,"BLK":0,
            "OP":0,"DP":0,
            # NEW: points-for/against while on court (for ratings/+/-)
            "PF_pts":0, "PA_pts":0,
            "PRAC":0
        }
...
def _aggregate_stats_table(rows):
    ...
    for r in rows:
        ...
        # --- Shooting contributions (player box stats) ---
        ...
        # Fouls (your PF/OF here are foul counts; +/- uses PF_pts/PA_pts below)
        ...
        # --- Participants (who was on offense/defense) ---
        try:
        ...
        # --- NEW: award team points-for/against for ratings and +/- ---
        pts_this_row = 0
        try:
            if r.get("result") == "Make":
                pts_this_row = 3 if r.get("is_three") else 2
        except Exception:
            pts_this_row = 0

        if pts_this_row:
            for nm in (op or []):
                n = _norm_force(nm); _ensure(n)
                stats[n]["PF_pts"] += pts_this_row
            for nm in (dp or []):
                n = _norm_force(nm); _ensure(n)
                stats[n]["PA_pts"] += pts_this_row

    # finalize simple derived fields
    ...
    # Return the full set (includes OP/DP/PF_pts/PA_pts/PRAC); we'll split for the two tables.
...
def _advanced_from_stats_rows(stats_full):
    ...
    Also computes ORtg, DRtg, NET, and +/- from PF_pts/PA_pts, OP, and DP.
    """
    out = []
    for r in (stats_full or []):
        ...
        # Ratings / impact
        PF_pts = int(r.get("PF_pts", 0) or 0)
        PA_pts = int(r.get("PA_pts", 0) or 0)
...
def compute_all(sd_sh, ed_sh, sz_sh, dr_sh, shtr, defs, asts, scrs, onb, offb, def_sh,
    ...
    try:
        ...
        # ADVANCED table (OP/DP + rate metrics + ratings)
