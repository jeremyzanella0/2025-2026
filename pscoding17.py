# pscoding17
# Mark 17 - updated after stage 2 of testing (1 full wnba game)
...
def parse_player_def(token: str):
    ...
    if token.startswith("(") and token.endswith(")"):
        ...
        action_type = None
        # ADD: 'bd' (backdoor cut)
        for key in ("pn", "fl", "bk", "awy", "crs", "wdg", "rip", "ucla", "stg", "ivs", "elv", "bd"):
            if key in inner:
                action_type = key
                break
        if action_type:
...
def format_player_list(players):
    ...
    Accepts raw ids or 'rot' ids. Uses norm_id() for final printing.
    """
    if not players:
        return "Unknown"
    if isinstance(players, str):
        return f"Player {norm_id(players)}"
    ids = [norm_id(p) for p in players if p is not None]
    if len(ids) == 0:
        return "Unknown"
    if len(ids) == 1:
...
def _defenders_to_parts(def_list, off_player=None):
    ...
    for d in (def_list or []):
        ...
        if _is_rot_token(s):
            ...
            parts.append(f"Player {norm_id(s)}")
    return parts
...
def coverage_phrase(def_list, off_player=None):
    ...
    Preferred coverage phrase builder that RETURNS A LEADING SPACE when non-empty:
      - def_list is None  -> unknown coverage
            -> '' normally; ' against the zone' if zone is active (unknown who, but in zone)
...
def print_pass(from_player, from_def, to_player, to_def, inbound=False):
    ...
    fp, fp_defs_fallback = _split_off_and_defs(from_player)
    tp, tp_defs_fallback = _split_off_and_defs(to_player)
...
def _format_screen_assist_suffix(screen_assist_by):
    ...
    if isinstance(screen_assist_by, str):
        return f"(screen assist by Player {norm_id(screen_assist_by)})"
    screeners = [norm_id(s) for s in screen_assist_by if s]
    if not screeners:
        return None
    if len(screeners) == 1:
...
def print_shot_line(player, def_list, action, last_passer=None, screen_assist_by=None):
    ...
    Shot printer that respects tri-state coverage:
      ...
      [...] -> ' guarded by ...'
    """
    player = norm_id(player)
    made = action in ("+","++")
    verb = "makes the shot" if made else "misses the shot"

    suffixes = []
    if action == "++" and last_passer:
        suffixes.append(f"(assisted by Player {norm_id(last_passer)})")
    ...
    cov = coverage_phrase(def_list, player)
    print(f"Player {player}{cov} {verb}{tail}{end}")
...
def handle_shot_with_screen_assists(player, def_list, action, last_passer=None, last_screener=None, pnr_shot=False):
    ...
    last_shot_defenders = norm_list(def_list)
    ...
    if made:
        if pnr_shot and last_screener:
            ...
            if isinstance(last_screener, (list, tuple)):
                screen_asst = [norm_id(s) for s in last_screener if s is not None]
            else:
                screen_asst = norm_id(last_screener)
        else:
            if (
                pending_offball_sa
                and norm_id(player) == norm_id(pending_offball_sa.get("recipient"))
                and action == "++"
                and last_passer
            ):
                screeners = pending_offball_sa.get("screeners") or []
                screen_asst = [norm_id(s) for s in screeners if s]
    ...
    while True:
        ...
        while i < len(parts):
            ...
            if token.startswith("(") and token.endswith(")"):
                ...
                if action_type in ("pn", "fl", "bk", "awy", "crs", "wdg", "rip", "ucla", "stg", "ivs", "elv", "bd"):
