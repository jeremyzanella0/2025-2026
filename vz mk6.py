...
#  - Applying subfilters ONLY when they are selected (None/[] => no-op)
...
ONBALL_ACTION_SUBFILTERS = {
    # show these when on-ball action == ...
    ...
    "kp":  ["ball_handler", "ball_handler_defender"],  # Keep
    ...
    "p":   ["ball_handler", "ball_handler_defender"],  # Post up
    "h":   ["ball_handler", "ball_handler_defender"],  # Halfcourt bring
}

OFFBALL_ACTION_SUBFILTERS = {
    # off-ball action code -> subfilters to show
    # "coming_off" is the mover; plus optional screener + defenders
...
# --- Safe, utility predicates for filtering ---
def _col_ok(df, name):  # no pandas import here yet in Section 1 elsewhere
    try:
...
def _isin_or_all(series, values):
    """If values is falsy (None, '', [], etc.), return 'all True' mask; else series.isin(values)."""
    import pandas as _pd  # local import keeps Section 1 lightweight at import time
    if not values:
...
def apply_action_filters(df,
    ...
    # ---------------- On-ball ----------------
    if on_action:
        # Expect a normalized on-ball action code (e.g., 'd', 'pnr', 'pnp', etc.)
        if _col_ok(df, "on_ball_action"):
            mask &= (df["on_ball_action"] == on_action)
        else:
            return df.iloc[0:0]  # column missing => nothing matches the requested on-ball action
        ...
        # PNR-style extras
        if on_action in ("pnr", "pnp", "rs", "rj", "slp", "gst"):
    ...
    if off_action:
        if _col_ok(df, "off_ball_action"):
            mask &= (df["off_ball_action"] == off_action)
        else:
            return df.iloc[0:0]
...
# Convenience helpers for downstream sections (robust stats/legend rendering)
...
def _coverage_match_any(filter_set, coverage_items):
    """
    Match on coverage code or label. Accepts things like 'sw' or 'switch' or 'Switch'.
    """
    if not filter_set:
        return True
    cov_labels = []
...
def _onball_action_satisfies_subfilters(action: dict, subf: dict) -> bool:
    ...
    # Ball handler & defender (common across many types)
    ...
    # Screener(s) + defenders for PNR family, rescreens, slips/ghosts/rejects
    scr_names = []
    scr_def_names = []
...
def row_passes_shooting_filters(row: dict, filters: dict) -> bool:
    ...
    assister, screen assister, on-ball action (+subfilters), off-ball action (+subfilters),
    defense Man/Zone).
    ...
    The 'filters' dict can contain:
      {
        ...
        # On-ball:
        "onball":     set[str] of codes (e.g., {"pnr","pnp","d"}),
        "on_bh":          set[str],
        "on_bh_def":      set[str],
        "on_screener":    set[str],
        "on_screener_def":set[str],
        "on_cov":         set[str],   # matches label or code (e.g., {"sw","Switch","Ice"})

        # Off-ball:
        "offball":    set[str] of codes (e.g., {"pn","bd"}),
        "off_mover":       set[str],
        "off_mover_def":   set[str],
        "off_screener":    set[str],
        "off_screener_def":set[str],
    ...
    # ----- On-ball action + subfilters -----
    if filters.get("onball"):
        sel = {c.lower() for c in (filters.get("onball") or [])}
        # any action of selected types present?
        candidates = [a for a in (onball or []) if (a.get("type","").lower() in sel)]
        if not candidates:
            return False

        # If subfilters provided, require that at least one candidate satisfies them
        subf = {
            "on_bh": filters.get("on_bh") or set(),
            "on_bh_def": filters.get("on_bh_def") or set(),
            "on_screener": filters.get("on_screener") or set(),
            "on_screener_def": filters.get("on_screener_def") or set(),
            "on_cov": filters.get("on_cov") or set(),
        }
        # only apply subfilter checks if any subfilter is non-empty
        if any(bool(v) for v in subf.values()):
            ok = any(_onball_action_satisfies_subfilters(a, subf) for a in candidates)
            if not ok:
                return False

    # ----- Off-ball action + subfilters -----
    if filters.get("offball"):
        sel = {c.lower() for c in (filters.get("offball") or [])}
        candidates = [a for a in (offball or []) if (a.get("type","").lower() in sel)]
        if not candidates:
            return False

        subf = {
            "off_mover": filters.get("off_mover") or set(),
            "off_mover_def": filters.get("off_mover_def") or set(),
            "off_screener": filters.get("off_screener") or set(),
            "off_screener_def": filters.get("off_screener_def") or set(),
        }
        if any(bool(v) for v in subf.values()):
...
def empty_shooting_filters() -> dict:
    ...
    return {
        ...
        "on_bh": set(), "on_bh_def": set(), "on_screener": set(), "on_screener_def": set(), "on_cov": set(),
        "off_mover": set(), "off_mover_def": set(), "off_screener": set(), "off_screener_def": set(),
...
def create_zone_chart(shots=None):
    """
    Build the hot/cold zone chart. If 'shots' is provided, it should already be a list of
    plottable shots (or raw rows—both accepted). If None, we safely load all rows.
    """
    # Accept either filtered shots or None (default: all)
    if shots is None:
        possession_rows = safe_load_data()
        shots = rows_to_shots(possession_rows)
    else:
        # Allow callers to pass raw rows or plottable shots; normalize to shots
        if shots and isinstance(shots[0], dict) and ("x" in shots[0]) and ("y" in shots[0]) and ("result" in shots[0]):
            # could be either; ensure result is Make/Miss and x/y are valid
            shots = rows_to_shots(shots)
        else:
            shots = rows_to_shots(shots)
...
app.layout = html.Div(
    ...
    children=[
                            ...
                            [
                                ...
                                html.Div(
                                    [
                                        ...
                                        # Legend key placed directly under the zones chart
        ...
        dcc.Store(id="options_cache", data={}),  # can cache dynamic dropdown options by action type
        # NEW: dedicated store for dynamic action subfilter state (keeps stats intact when UI updates)
...
# ---------- core row filtering (base filters only) ----------
def _filter_rows(rows,
    ...
    if "man" in def_norm or "man to man" in def_norm: def_norm.add("man to man")
    if "zone" in def_norm: def_norm.add("zone")
    ...
    for r in rows:
        ...
        if drill_sizes and (r.get("drill_size") or "").lower() not in {s.lower() for s in (drill_sizes or [])}:
            continue
        if drills_full and (r.get("drill") or "").lower() not in {s.lower() for s in (drills_full or [])}:
            continue
        ...
        if not for_stats_tab:
            if shooters:
                ...
                if not shots.intersection({n.lower() for n in shooters}): continue
            if defenders:
                defs = set([n.lower() for n in _split_names(r.get("defenders_raw"))])
                if not defs.intersection({n.lower() for n in defenders}): continue
            if assisters:
                asts = set([n.lower() for n in _split_names(r.get("assister_raw"))])
                if not asts.intersection({n.lower() for n in assisters}): continue
            if screen_assisters:
                sas = set([n.lower() for n in _split_names(r.get("screen_assisters_raw"))])
                if not sas.intersection({n.lower() for n in screen_assisters}): continue
            if onball and not set(r.get("onball_set") or []).intersection({a.lower() for a in onball}):
                continue
            if offball and not set(r.get("offball_set") or []).intersection({a.lower() for a in offball}):
                continue
...
def _aggregate_stats_table(rows):
    ...
    cols = ["Player","FGM","FGA","FG%","2PM","2PA","2P%","3PM","3PA","3P%",
            "AST","DRB","ORB","TRB","LBTO","DBTO","TO","PF","OF","F","STL","DEF","BLK","OP","DP"]
...
_PNR_LIKE = {"pnp","gst","slp","ho","kp","rj"}
...
# ---------- Build available facet options from rows ----------
...
@callback(
    ...
    prevent_initial_call='initial_duplicate'   # <<< FIX: allow_duplicate requires this
)
def populate_onball_subfilter_options(sd, ed, sz, dr, shtr, defs, asts, scrs, onball, offball, def_sh):
    rows = _rows_full()
    prelim = _filter_rows(
        rows,
    ...
    coll = _collect_onball_facet_options(prelim)
...
def _row_matches_onball_facets(row, need_types, bh_v, bhd_v, scr_v, scrd_v, cov_v):
    ...
    sel_bh   = set(map(N, bh_v or []))
    sel_bhd  = set(map(N, bhd_v or []))
    sel_scr  = set(map(N, scr_v or []))
    sel_scrd = set(map(N, scrd_v or []))
    sel_cov  = set(cov_v or [])

    def match_cov(covs):
        if not sel_cov: return True
        have = set()
        for c in (covs or []):
            lbl = c.get("label"); onto = N(c.get("onto",""))
            have.add(f"{lbl}→{onto}" if onto else lbl)
        return bool(have & sel_cov)

    for a in actions:
        t = (a.get("type") or "").lower()
        if t in need_types or (t in _PNR_LIKE and "pnr" in need_types):
...
@app.callback(
    ...
    Output("flt_onball_bh", "value", allow_duplicate=True),
    Output("flt_onball_bh_def", "value", allow_duplicate=True),
    Output("flt_onball_scr", "value", allow_duplicate=True),
    Output("flt_onball_scr_def", "value", allow_duplicate=True),
    Output("flt_onball_cov", "value", allow_duplicate=True),
...
# ===== compute main views (includes on-ball subfilters) =====
@app.callback(
    ...
    # On-ball subfilters
    State("flt_onball_bh","value"),
    State("flt_onball_bh_def","value"),
    State("flt_onball_scr","value"),
    State("flt_onball_scr_def","value"),
    State("flt_onball_cov","value"),
...
def compute_all(sd_sh, ed_sh, sz_sh, dr_sh, shtr, defs, asts, scrs, onb, offb, def_sh,
                on_bh, on_bhd, on_scr, on_scrd, on_cov,
    ...
    try:
        ...
        need_types = set((onb or []))
        if need_types:
            shooting_rows = [
                r for r in shooting_rows
                if _row_matches_onball_facets(
                    r, need_types,
                    on_bh if isinstance(on_bh, list) else ([on_bh] if on_bh else []),
                    on_bhd if isinstance(on_bhd, list) else ([on_bhd] if on_bhd else []),
                    on_scr if isinstance(on_scr, list) else ([on_scr] if on_scr else []),
                    on_scrd if isinstance(on_scrd, list) else ([on_scrd] if on_scrd else []),
                    on_cov if isinstance(on_cov, list) else ([on_cov] if on_cov else []),
                )
            ]
    ...
    try:
        ...
        header_top = html.Div([
            ...
            html.Div([
                ...
                html.Span(", ".join(dp_list) if op_list else "—", style={"color":"#333"})
            ], style={"marginTop":"2px"})
