# vz_mk2.py — Adds possession details to shots
...
from dash import callback_context  # for close button detection
from dash import ALL  # <-- added for pattern-matching Input
...
# -------------------------
# Precompute geometry used by zone tests (no per-point searches)
# -------------------------
...
# -------------------------
# Zone classifier (UNCHANGED math, but using precomputed intersections)
# -------------------------
...
def point_in_zone(x, y, zone_id):
    ...
    # Zone 11: Left wing 3
        ...
        # diagonal limiter (precomputed intersection)
        left_start_x_diag, left_start_y_diag = LEFT_DIAG_START
        def y_diagonal_line(xq):
            if xq <= LEFT_DIAG_IX:
                return LEFT_DIAG_IY
            else:
                return left_start_y_diag + LEFT_DIAG_SLOPE * (xq - left_start_x_diag)
    ...
    # Zone 13: Right wing 3
        ...
        def y_diagonal_line(xq):
            if xq >= RIGHT_DIAG_IX:
                return RIGHT_DIAG_IY
            else:
                return right_start_y_diag + RIGHT_DIAG_SLOPE * (xq - right_start_x_diag)
        if y <= y_diagonal_line(x): return False
        return True
...
def add_zone_fill(fig, zone_id, rgba="rgba(255,255,0,0.35)", step=0.25):
    # step 0.25 ft → smoother edges to match vz_mk1
...
# ------------- Shot chart (add optional highlights)
...
def create_shot_chart(shots, highlight_coords=None):
    ...
    # Highlight boxes around selected possession shots
    if highlight_coords:
        # draw gray rectangles behind the markers at each coord
        L = 1.2  # box size in feet
    ...
    # ensure clicks always fire on this chart
...
def create_zone_chart():
    ...
    # Shade every zone based on FG% and place 2-line text
    zone_centers = {
...
# ---- Instant initial figures (no blank state)
...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Store(id="sel_pos", data=[]),  # holds coords of shots to highlight
    ]
)

# --- FAST: update only the SHOT CHART when selection changes (or on refresh)
@app.callback(
...
# --- Keep zone chart & status on the 20s timer ONLY (no selection dependency)
@app.callback(
...
# --- Details & selection management (pattern-matching close button) ---
@app.callback(
    [Output("shot_details", "children"),
     Output("sel_pos", "data")],
    [Input("shot_chart", "clickData"),
     Input({"type":"close_details","idx":ALL}, "n_clicks")],  # <-- safe even when none exist initially
    prevent_initial_call=False
)
def show_shot_details(clickData, close_clicks):
    # Handle "×" close: clear panel + highlights
    ...
    try:
        ...
        x_clicked, y_clicked = float(p["x"]), float(p["y"])  # plotted values
        ...
        if os.path.exists(DATA_PATH):
            ...
            for r in (data or []):
                x = r.get("x"); y = r.get("y")
                try:
                    x = float(x) if str(x).strip() not in ("", "None") else None
                    y = float(y) if str(y).strip() not in ("", "None") else None
                except:
                    x = y = None
                res = r.get("result") or result_from_shorthand(r.get("possession",""))
                if x is not None and y is not None and (0 <= x <= COURT_W) and (0 <= y <= HALF_H) and res in ("Make","Miss"):
                    rows_for_plot.append({**r, "x":x, "y":y, "result":res})
        ...
        # Fraction display for shot number in possession
        ...
        # Coordinates to highlight for all shots in the possession
        pos_coords = [(rr["x"], rr["y"]) for rr in rows_for_plot if same_possession(rr)]

        def line(label, val):
            return html.Div([
                html.Span(label, style={"fontWeight":600, "marginRight":"6px"}),
                html.Span((val or ""), style={"whiteSpace":"pre-wrap"})
            ], style={"marginBottom":"6px"})

        details = html.Div([
            html.Button("×", id={"type":"close_details","idx":0},  # <-- dict id
                        style={"position":"absolute","top":"6px","right":"8px",
                               "border":"none","background":"transparent",
                               "fontSize":"20px","cursor":"pointer","lineHeight":"16px"}),
            html.H4("Shot Details", style={"marginTop": 0}),
            line("Shorthand:", r.get("possession")),
            html.Div(style={"height": "1px", "background": "#ddd", "margin": "8px 0"}),
            line("Play-by-Play (names):", (r.get("play_by_play_names") or r.get("play_by_play") or "")),
            html.Div(style={"height": "1px", "background": "#ddd", "margin": "8px 0"}),
            html.Div([
                html.Div([line("Result:", r.get("result"))], style={"flex":"1 1 0"}),
                html.Div([line("Distance (ft):", (f"{float(r.get('distance_ft')):.1f}" if r.get("distance_ft") is not None else ""))], style={"flex":"1 1 0"})
            ], style={"display":"flex", "gap":"24px"}),
            html.Div([
                html.Div([line("Practice Date:", r.get("practice_date"))], style={"flex":"1 1 0"}),
                html.Div([line("Drill:", r.get("drill_name"))], style={"flex":"1 1 0"}),
                html.Div([line("Shot # in Possession:", shot_num_display)], style={"flex":"1 1 0"})
            ], style={"display":"flex", "gap":"24px", "marginTop":"6px"}),
            html.Div(html.Em("Coming next: parsed shooter, on-ball defender, assister, actions (PnR, flare, etc.)"),
                     style={"color":"#666", "marginTop":"8px"})
        ], style={"position":"relative","border":"1px solid #ddd","borderRadius":"10px","padding":"14px"})

        return details, pos_coords

    except Exception as e:
        return html.Div(f"Details error: {e}", style={"color":"#b91c1c"}), []
