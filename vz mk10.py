...
# Column definitions for the Advanced table (initial version: only OP, DP).
...
def get_advanced_stats_columns():
    """
    Return the dash DataTable-ready columns for the Advanced table (Player, OP, DP).
    Requires ADVANCED_STATS_COLUMNS from Section 1.
    """
    cols = globals().get("ADVANCED_STATS_COLUMNS", [
        {"key": "Player", "name": "Player"},
        {"key": "OP", "name": "OP"},
        {"key": "DP", "name": "DP"},
    ])
...
def split_basic_and_advanced_rows(full_rows: list[dict]):
    ...
    Take aggregated per-player rows (which include OP/DP) and produce:
      ...
      - advanced_rows: rows shaped for the Advanced table (Player, OP, DP)
    Uses the Section 2 helper project_advanced_from_basic_rows(...).
    """
    if 'project_advanced_from_basic_rows' in globals():
        return project_advanced_from_basic_rows(full_rows or [])
    # Fallback if Section 2 helper isn't in scope for any reason
    basic = []
    adv = []
    for r in (full_rows or []):
        rr = dict(r); op = rr.pop("OP", 0); dp = rr.pop("DP", 0)
        basic.append(rr)
        adv.append({"Player": r.get("Player",""), "OP": op, "DP": dp})
    return basic, adv
...
def parse_offball_actions_from_pbp(lines):
    ...
    FIX: Tag each action with connected_to_shot=True only if it appears on the shot line.
    """
    stg_agg = None
    stg_seen_scr = set()
    stg_seen_cov = set()
    ...
    for ln in (lines or []):
        ...
        is_shot_line = _is_shot_line(ln)  # <-- shot connectivity
        ...
        if matched_key == "stg":
            ...
            # record connectivity if any stagger mention is on shot line
            if is_shot_line:
                stg_connected = True
            continue
        ...
        actions.append({
            ...
            "screeners": screeners,
            "coverages": covs,
            "connected_to_shot": bool(is_shot_line),  # <-- new flag
        })
...
# ---- On-ball action filter options (full names, no shorthand; no 'bring over')
ONBALL_OPTIONS = [
    {"label": "Drive",              "value": "d"},
    ...
    children=[
        ...
        dcc.Tabs(id="tabs", value="tab_shooting", children=[
            ...
            dcc.Tab(label="Stats", value="tab_stats", children=[
                ...
                # ---------- Stats table (BASIC: OP/DP removed)
                html.Div([
...
def _force_to_roster_name(piece: str) -> str:
    ...
    First try the strict Section-2 canonicalizer; if that fails, fall back to
    sliding windows resolved via _normalize_to_roster. Returns '' if unresolved.
    """
    s = (piece or "").strip()
    if not s:
        return ""

    # Primary: strict resolver from Section 2 (handles 'for Fontana', 'Boyd dribbles', etc.)
    nm = _strict_canon_name(s)
    if nm:
        return nm

    # Fallbacks: keep legacy behavior but insist on a full-name hit
...
# Canonical off-ball shorthand you use
_OFFBALL_CODES = {
    "bd", "pn", "fl", "awy", "hm", "crs", "wdg", "rip", "ucla", "stg", "ivs", "elv"
}

# Aliases so UI labels match codes
_OFFBALL_ALIAS_MAP = {
        ...
        if x is not None and y is not None and (0 <= x <= COURT_W) and (0 <= y <= HALF_H) and res in ("Make","Miss"):
            ...
            rows_for_plot.append({
                **rr,
                ...
                # NEW: cleaned player lists for modal display
...
def _filter_rows(rows,
    ...
    for r in rows:
        ...
        if not for_stats_tab:
            ...
            # On-ball filter uses connected set (codes + aliases)
            if onball:
...
def _normalize_display_name(nm: str):
    # Ensure displayed names are roster-exact
...
def _aggregate_stats_table(rows):
    ...
    for r in rows:
        ...
        for nm in (sps.get("def_fouls") or []):
            n = _norm_force(nm); _ensure(n); stats[n]["PF"] += 1
            if pday: prac_days[n].add(pday)
        for nm in (sps.get("off_fouls") or []):
            n = _norm_force(nm); _ensure(n); stats[n]["OF"] += 1
            if pday: prac_days[n].add(pday)
...
@app.callback(
    ...
    Output("advanced_stats_table","data"),  # <<< NEW: populate Advanced Stats table
...
def compute_all(sd_sh, ed_sh, sz_sh, dr_sh, shtr, defs, asts, scrs, onb, offb, def_sh,
    ...
    try:
        ...
        # ADVANCED table (just OP/DP)
        adv_cols = ["Player","OP","DP"]
        stats_adv = [{c: r.get(c, 0) for c in adv_cols} for r in stats_full]
