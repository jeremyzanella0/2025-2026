# pscoding12
# Mark 12 — starts to add off ball actions, starting with pin down. Also replaces ices with top locking for off ball actions
...
def parse_player_def(token: str):
    ...
    if token.startswith("(") and token.endswith(")"):
        inner = token[1:-1]
        action_type = None
        for key in ("pn",):  # off-ball screen types
            if key in inner:
                action_type = key
                break
        if action_type:
            ...
            if "/" in rest:
                parts = rest.split("/", 1)
                screener = parts[0]
                screener_def = re.match(r"\d*", parts[1]).group(0)
                after = parts[1][len(screener_def):]
    ...
    while True:
        ...
        while i < len(parts):
            ...
            if token.startswith("(") and token.endswith(")"):
                ...
                if action_type == "pn":
                    print(f"Player {ball} {defender_text(balldef,ball)} comes off a pin down from Player {screener} {defender_text(screendef,screener)}")
                    if after:
                        for match in re.finditer(r"(ch|ct|sw|bz|tl|cs)", after):  # REPLACE ICE with TL
                            code = match.group(0)
                            if code == "ch":
                                print(f"Player {balldef} chases over the pin down")
                            elif code=="ct":
                                print(f"Player {balldef} cuts under the pin down")
                            elif code=="sw":
                                print(f"Player {screendef} switches onto {ball}")
                            elif code=="bz":
                                print(f"Players {balldef} and {screendef} double team {ball}")
                            elif code=="tl":
                                print(f"Player {balldef} top locks {ball}, forcing them away from the screen")  # FIXED
                            elif code=="cs":
                                print(f"Player {ball} is caught on the pin down")
                i += 1
                continue
            ...
            if any(k in token for k in ("pnr","pnp","slp","gst","rj","dho","ho")):
                ...
                # Handle trailing defensive codes
                defenses = []
                trailing = ""
                if after:
                ...
                for def_code in defenses:
                    if def_code=="ch":
                        ...
                        print(f"Players {balldef} and {screendef} ice the screen/handoff")  # On-ball icing remains
                    elif def_code=="cs":
                        print(f"Player {ball} is caught on the screen/handoff")

                # Handle trailing drive/dribble/shot
                while trailing:
