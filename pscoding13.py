# pscoding13
# Mark 13 — expands off-ball actions beyond pin down
# Adds: flare (fl), back screen (bk), away screen (awy), cross screen (crs), wedge (wdg), rip (rip), ucla (ucla)
...
def parse_player_def(token: str):
    """
    Parse a token into ball handler, ball defender, action type, screener, screener_def, and after-action codes.
    Supports PNR, PNP, SLP, GST, RJ, DHO, HO, slash actions, numeric tokens, and Off-Ball parentheses.
    ...
    if token.startswith("(") and token.endswith(")"):
        inner = token[1:-1]
        action_type = None
        # include all off-ball screen types
        for key in ("pn", "fl", "bk", "awy", "crs", "wdg", "rip", "ucla"):
            if key in inner:
                action_type = key
                break
        if action_type:
            # split into ball part and rest
            ball_part, rest = inner.split(action_type, 1)
            ball_handler, ball_def = None, None
            if "/" in ball_part:
                # ball and its defender
                ball_handler, ball_def = ball_part.split("/", 1)
            screener, screener_def, after = None, None, rest
            if "/" in rest:
                parts = rest.split("/", 1)
                screener = parts[0]
                # screener_def may be digits or rot... we'll grab leading digits
                m = re.match(r"(\d*)", parts[1])
                screener_def = m.group(1) if m else None
                after = parts[1][len(screener_def):] if screener_def else parts[1]
...
def defender_text(def_player, off_player=None):
    """Return human readable defender text.
    Supports None, string, list, and rotX tokens.
    """
    if not def_player:
        return "wide open"
...
def print_offball_screen(action_type, ball, balldef, screener, screendef, after):
    """Prints standardized messages for off-ball screens.
    action_type: one of pn, fl, bk, awy, crs, wdg, rip, ucla
    ball: ball handler (string)
    balldef: ball's defender (string or None)
    screener: screener player (string)
    screendef: screener's defender (string or None)
    after: trailing action codes (string)
    """
    labels = {
        "pn": "pin down",
        "fl": "flare screen",
    ...
    # ball may be None for some malformed tokens; guard against that
    ...
    if after:
        for match in re.finditer(r"(ch|ct|sw|bz|tl|cs)", after):
            code = match.group(0)
            if code == "ch":
                print(f"Player {balldef_txt} chases over the {label}")
            elif code == "ct":
                print(f"Player {balldef_txt} cuts under the {label}")
            elif code == "sw":
                # sw means switch: typically screener_def switches onto ball
                print(f"Player {screendef_txt} switches onto {ball_txt}")
            elif code == "bz":
                print(f"Players {balldef_txt} and {screendef_txt} double team {ball_txt}")
            elif code == "tl":
                print(f"Player {balldef_txt} top locks {ball_txt}, forcing them away from the {label}")
            elif code == "cs":
...
def main():
    while True:
        ...
        while i < len(parts):
            token = parts[i]
            # First process defensive/rebound tokens
            ...
            # HELP/SWITCH handling (hp / sw), should work whether part of slash or standalone
            if token.startswith(("hp","sw")):
                ...
                # infer last possessor if missing
            ...
            # Off-ball action (parentheses): now supports multiple types
            if token.startswith("(") and token.endswith(")"):
                ball, balldef, action_type, screener, screendef, after = parse_player_def(token)
                if action_type in ("pn","fl","bk","awy","crs","wdg","rip","ucla"):
                    # generic off-ball printer
                    print_offball_screen(action_type, ball, balldef, screener, screendef, after)
                i += 1
                continue

            # PNR / PNP / SLP / GST / RJ / DHO / HO handling (unchanged from Mark 12)
            if any(k in token for k in ("pnr","pnp","slp","gst","rj","dho","ho")):
                ...
                # Handle trailing defensive codes (keep both on-ball and off-ball tokens like ice/tl etc.)
                defenses = []
                trailing = ""
                if after:
                ...
                # trailing actions handling (drive, dribble, shot)
                while trailing:
            ...
            # Plain token (player number without slash or action)
