# pscoding9
# Mark 9 — starts to add in on ball actions with pick and roll and pick and pop
...
def parse_player_def(token: str):
    ...
    Works for PNR, PNP, slash actions, and numeric tokens.
    """
    action_type = None
    if "pnr" in token:
        action_type = "pnr"
...
def process_defensive_tokens(token, last_player, last_defender, last_shooter, off_player=None, def_list=None):
    ...
    Works even if token has a player prefix like '5/6^or'.
    """
    base_token = token
    if "/" in token:
        parts = token.split("/")
    ...
    while True:
        ...
        while i < len(parts):
            ...
            if token.startswith(("hp","sw")):
                ...
                if match:
                    token, trailing = match.group(1)+match.group(2), match.group(3)
                    if trailing:
                        parts.insert(i+1, trailing)
            ...
            # PNR / PNP
            if "pnr" in token or "pnp" in token:
                ...
                last_screener = screener

                # Handle defenses including rescreens
                if after:
                    defenses = after.split(",")  # split multiple defenses
                    ...
                    # Rescreens
                    for idx, res_def in enumerate(rest_defs):
                        print(f"Player {screener} guarded by {screendef} rescreens for {ball} guarded by {balldef}")
                        if res_def == "ch":
                    ...
                    after = ""  # clear defenses after processing

                # Handle drives or dribbles
                ...
                if after:
                    ...
                    handle_shot(ball,defender_list,after,last_passer,last_screener,pnr_shot=True)
