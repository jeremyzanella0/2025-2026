...
# key: field in each player row; name: header label shown in the UI.
...
ADVANCED_STATS_KEYS = ["eFG%", "PPS", "AST%", "TOV%", "AST/TO", "OP", "DP"]
...
# ---- NEW (mk11-advstats): build extended advanced row locally if needed ----------
def _build_advanced_row_extended(player_record: dict) -> dict:
    """
    Construct an Advanced-table row including OP, DP and the five derived metrics.
    Assumes compute_adv_metrics_for_row(...) has already populated the metrics.
    """
    rec = {
...
def split_basic_and_advanced_rows(full_rows: list[dict]):
    ...
    # 1) Compute advanced metrics onto each player row if helper exists.
    ...
    if 'project_advanced_with_metrics' in globals():
        ...
        # project_advanced_with_metrics may only include OP/DP; re-build with metrics if needed
        if adv and "eFG%" not in adv[0]:
...
_OFFBALL_TYPES = {
    "bd":  {"label": "Backdoor cut", "keys": [r"\bbackdoor\b", r"backdoor\s+cut"]},
    "pn":  {"label": "Pin down", "keys": [r"\bpin\s*down\b", r"\bpindown\b"]},
    "fl":  {"label": "Flare screen", "keys": [r"\bflare\s+screen\b"]},  # fixed typo
    "bk":  {"label": "Back screen", "keys": [r"\bback\s+screen\b"]},
    "awy": {"label": "Away screen", "keys": [r"\baway\s+screen\b"]},
    "hm":  {"label": "Hammer screen", "keys": [r"\bhammer\s+screen\b"]},
    "ucla":{"label": "UCLA screen", "keys": [r"\bucla\s+screen\b"]},
...
def parse_offball_actions_from_pbp(lines):
    ...
    Tag each action with connected_to_shot=True only if it appears on the shot line.
    """
    stg_agg = None
    stg_seen_scr = set()
    stg_seen_cov = set()
...
OFFBALL_OPTIONS = [
    ...
    {"label":"Flare Screen","value":"fl"},  # fixed label
...
# FIX: values must be {"Man","Zone"} to match filter predicates
DEFENSE_OPTIONS = [
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                # Legend key under the zones chart
            ...
            dcc.Tab(label="Stats", value="tab_stats", children=[
                ...
                html.Div([
                    ...
                    dash_table.DataTable(
                        ...
                        columns=(get_basic_stats_columns() if 'get_basic_stats_columns' in globals()
                                 else [{"name":c,"id":c} for c in [
                ...
                ], style={"border":"1px solid #eee","borderRadius":"8px","padding":"8px","background":"white","marginBottom":"10px"}),

                # ---------- Advanced Stats table (OP/DP + derived metrics)
                html.Div([
                    html.Div("Advanced Stats", style={"fontSize":"18px","fontWeight":700,"marginBottom":"6px"}),
                    dash_table.DataTable(
                        id="advanced_stats_table",
                        columns=(get_advanced_stats_columns() if 'get_advanced_stats_columns' in globals()
                                 else [{"name":c,"id":c} for c in ["Player","OP","DP","eFG%","PPS","AST%","TOV%","AST/TO"]]),
                        data=[],
                        sort_action="native",
                        style_table={"overflowX":"auto"},
                        style_cell={"fontFamily":"Arial","fontSize":"13px","padding":"6px","textAlign":"center"},
...
def _force_to_roster_name(piece: str) -> str:
    """
    Coerce any messy fragment into an exact roster Full Name.
    Primary: _strict_canon_name (Section 2). Fallbacks: _normalize_to_roster windows.
...
_ONBALL_FINISH_CODES = {c.lower() for c in ONBALL_ACTION_CODES}  # from Section 2

def _onball_codes_connected_to_shot(short_text: str) -> set[str]:
    """
    On-ball codes connected to the shot = any on-ball codes that appear
    in the finishing token (first token with + or -) OR earlier tokens.
    """
    out = set()
...
# Short-code ↔ human label expansion (for filtering)
_ONBALL_ALIAS_MAP = {
...
# finishing token and shooter "pair" (e.g., 4/5) from shorthand
...
def _finishing_token_info(short_text: str):
    ...
    for i, tok in enumerate(toks):
        ...
        if "+" in tl or "-" in tl:
            ...
            if m:
                shooter_pair = m.group(1)  # e.g., "4/5"
            break
    return toks, finish_idx, shooter_pair

# parse parentheses segments up to finishing token and extract the RECEIVER/CUTTER (left side)
...
def _receivers_from_segment(seg: str):
    ...
    # left side before the action code is the receiver/cutter
...
def _offball_codes_connected_to_shot(short_text: str) -> set[str]:
    """
    Off-ball codes connected to the shot:
    - action appears in parentheses before (or at) the finishing token, and
    - the finishing shooter equals the receiver/cutter (left-side pair).
    """
    out = set()
...
def _participants_clean(pbp_src: str, short: str):
    """
    Wrap participants_for_possession with strict canonicalization so OP/DP
    contain only exact roster names.
    """
    try:
...
def _rows_full():
    ...
    for rr in (data or []):
        ...
        # PBP (prefer names version), shorthand (use dedicated field via Section 2 helper)
        ...
        short = _row_shorthand_text(rr)  # <<< FIX: use dedicated shorthand keys
        ...
        # A) On-ball/off-ball actions **only on the shot line** (display-only)
        ...
        # Expand to include human-readable aliases
        ...
        # Clean OP/DP for display (and reuse elsewhere if needed)
        ...
        # If the shorthand embeds a zone tag like '2-3[ ... ]', trust it.
...
def _rows_stats_all():
    """
    Return rows for the Stats tab that include *all* possessions.
    For true shot rows, also populate result/is_three and shooter/assister/screeners
    exactly like _rows_full() so the aggregator can count FGM/FGA/PTS/etc.
    """
    rows_for_stats = []
    if not os.path.exists(DATA_PATH):
    ...
    for rr in (data or []):
        ...
        # Defense label (same logic as _rows_full, but robust to zero-shot)
        try:
            idx = rr.get("shot_index") or 1
            def_label = defense_label_for_shot(pbp_src_for_roles, idx) or "Man to Man"
        except:
        ...
        # Try to detect a valid shot (so we can populate shot fields for the aggregator)
        x = y = None
        is_three = None
        result = None
        shooter_raw = ""
        ...
        if x is not None and y is not None and 0 <= x <= COURT_W and 0 <= y <= HALF_H:
            # We treat this as a shot possession; carry same fields as _rows_full
            result = rr.get("result") or result_from_shorthand(pbp_raw_src or short or "")
            is_three = _is_three(x, y)

            # roles from PBP, with same fallbacks
            idx = rr.get("shot_index") or 1
            try:
        ...
        rows_for_stats.append({
            ...
            # shot fields (present only when it’s a valid shot)
            "result": result,
            "is_three": is_three,
            "shooter_raw": shooter_raw,
            "assister_raw": assister_raw,
...
def _filter_rows(rows,
    ...
    for r in rows:
        ...
        if not for_stats_tab:
            # Canonicalize before comparing to filters
            if shooters:
            ...
            # On-ball filter uses connected set (codes + aliases) — only makes sense for shots
            if onball:
            ...
            # Off-ball filter uses connected set (shooter must be receiver/cutter)
            if offball:
...
def _aggregate_stats_table(rows):
    def _norm_force(nm: str):
        return _force_to_roster_name((nm or "").strip())
    ...
    for r in rows:
        ...
        # Screen assists: credit only on made shots
        if r.get("result") == "Make":
        ...
        # Fouls: keep tallies if your table adds these later
        ...
        # Use cleaned participants
        try:
            op, dp = _participants_clean(pbp_src, short or "")
        except:
            op, dp = ([], [])
        for nm in (op or []):
    ...
    # Return the full set (includes OP/DP/PRAC); we'll split for the two tables.
    cols = ["Player","PTS","FGM","FGA","FG%","2PM","2PA","2P%","3PM","3PA","3P%",
            "AST","SA","DRB","ORB","TRB","LBTO","DBTO","TO","STL","DEF","BLK","OP","DP","PRAC"]
    out_rows = []
...
# ---------- Advanced rows (eFG%, PPS, AST%, TOV%, AST/TO) from aggregated rows ----------
def _advanced_from_stats_rows(stats_full):
    """
    Build Advanced Stats rows from the aggregated per-player rows produced by
    _aggregate_stats_table(...). Uses OP as the possession denominator for rates.
    """
    out = []
    for r in (stats_full or []):
        FGA = int(r.get("FGA", 0) or 0)
        FGM = int(r.get("FGM", 0) or 0)
        TPM = int(r.get("3PM", 0) or 0)
        PTS = int(r.get("PTS", 0) or 0)
        AST = int(r.get("AST", 0) or 0)
        TO  = int(r.get("TO",  0) or 0)
        OP  = int(r.get("OP",  0) or 0)
        DP  = int(r.get("DP",  0) or 0)
...
def populate_filter_options(_tab_value):
    ...
    # Canonicalize all names before building options so filters match roster exactly
...
@app.callback(
    ...
    Output("advanced_stats_table","data"),  # Advanced now includes formulas
...
def compute_all(sd_sh, ed_sh, sz_sh, dr_sh, shtr, defs, asts, scrs, onb, offb, def_sh,
                ...
                sel_coords):
    try:
        # Shot-only rows for the Shooting tab visuals
        rows = _rows_full()
        ...
        # All possessions (including zero-shot) for the Stats tab
        ...
        # Aggregate once, then split into two tables
        stats_full = _aggregate_stats_table(stats_rows)

        # BASIC table (no OP/DP; must match Section 4 column order)
        ...
        # ADVANCED table (OP/DP + rate metrics computed from filtered set)
...
# Fill legend labels whenever the zone chart updates
@app.callback(
...
def update_zone_legend(_zone_fig):
    # Representative zone IDs by tier (see Section 4 tiering rules)
    close_bins = _legend_bins_for_zone(1)   # close tier
    mid_bins   = _legend_bins_for_zone(6)   # mid tier
    three_bins = _legend_bins_for_zone(12)  # three tier
