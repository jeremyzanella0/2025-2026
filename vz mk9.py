...
def show_shot_details(clickData, close_clicks):
    ...
    try:
        ...
        # ---- NEW: Only keep on-ball actions that are CONNECTED to THIS SHOT's shooter
        # A connected action is one where the shooter is the BH/keeper OR the final receiver of the action.
        def _connected_to_shooter(a, shooter_name: str) -> bool:
            s = (shooter_name or "").strip().lower()
            if not s:
                return False
            # candidates that can match the shooter
            cand_fields = [
            ...
            # also inspect any multi-screener lists
            for srec in (a.get("screeners") or []):
                cand_fields.append(srec.get("name",""))
                cand_fields.append(srec.get("def",""))

            # Heuristic: shooter must be the *offensive* side of the action:
            # prefer matches on bh/keeper/receiver/intended (not defenders)
            ...
            # As a safe fallback, allow exact match anywhere (handles odd phrasing)
        ...
        # If no assister parsed but a DHO/HO fed the shooter, infer assister from that connected action
        if not assister:
